...
(function(){
  ...
  const FRIEND_FALLBACK=Object.fromEntries(Object.entries(TRIP_CONFIG.participants?.identities||{}).map(([key,value])=>[key,`${value.emoji||''} ${value.name}`.trim()]));
  ...
  window.saveExpense=function(){
    ...
    const data={item,details,category:null,total,paidBy,type:personal?'personal':'shared',split:personal?[consumedBy]:split,splitMode,shares:personal?null:shares,consumedBy:personal?consumedBy:null,createdAt:now,updatedAt:now};
