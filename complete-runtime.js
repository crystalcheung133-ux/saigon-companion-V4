...
(function(){
  ...
  function showCompletionNoticeOnce(){
    ...
    alert('Trip completed. You can still browse the itinerary, guide, moments and expenses. Enter Studio Mode and choose Reopen Trip to make changes.');
  ...
  function isMutationControl(el){
    ...
    return /saveChecklist|openExpenseModal|saveExpense|editExpense|deleteExpense|openMomentsModal|openPlannedMomentCapture|saveMoments|editMoment|deleteMoment|openUnexpectedModal|saveUnexpected|openTimelineEditor|applyTimelineEdit|openTimelineDelete|confirmTimelineDelete|openTimelineMove|applyTimelineMove/.test(call);
  }
  ...
  function installGuards(){
    ['saveChecklist','openExpenseModal','saveExpense','editExpense','deleteExpense','openMomentsModal','openPlannedMomentCapture','saveMoments','editMoment','deleteMoment','openUnexpectedModal','saveUnexpected','openTimelineEditor','applyTimelineEdit','openTimelineDelete','confirmTimelineDelete','openTimelineMove','applyTimelineMove'].forEach(wrap);
  }
  ...
  window.completeTrip=function(){
    ...
    if(getFriend()!==ADMIN_USER || typeof window.isAdminMode!=='function' || !window.isAdminMode()){ alert('Enter Studio Mode to complete the trip.'); return false; }
  ...
  window.reopenTrip=function(){
    if(!completed) return true;
    if(getFriend()!==ADMIN_USER || typeof window.isAdminMode!=='function' || !window.isAdminMode()){ alert('Enter Studio Mode to reopen the trip.'); return false; }
