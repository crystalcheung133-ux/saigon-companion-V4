...
(function(root){
  ...
  root.openBookingEditor=function(id){
    ...
    const dayInfo=b.eventId&&b.day?`<a class="booking-day-button booking-day-button--detail" href="day.html?day=${encodeURIComponent(b.day)}#${encodeURIComponent(b.eventId)}">DAY ${esc(b.day)} · Open timeline</a>`:'';
    host.innerHTML=`<div class="booking-editor-head"><p class="kicker">Shared booking</p><h2>${esc(b.title)}</h2><p class="booking-place-link">${b.placeId?`<a href="place.html?id=${encodeURIComponent(b.placeId)}">Open Guide card →</a>`:'Independent booking record'}</p>${dayInfo}</div><div class="booking-readonly-grid">${readonlyField('Booking status',label(b.status))}${readonlyField('Date',formatDate(b.date))}${readonlyField('Time',b.time)}${readonlyField('Booking name',b.bookingName)}${readonlyField('Deposit',deposit)}${readonlyField('Booking option',b.bookingMethod)}${readonlyField('Contact details',contactInfo,true,true)}${secondaryInfo}${readonlyField('Online booking / website',onlineInfo,true,true)}${readonlyField('Notes',b.notes,true)}</div><div class="booking-view-actions"><button class="btn booking-edit-btn" type="button" onclick="editBookingEditor('${esc(id)}')">Edit booking</button></div>`;
  ...
  root.closeBookingModal=function(){document.getElementById('bookingModal')?.classList.remove('show');document.getElementById('tripModal')?.classList.remove('show');};
